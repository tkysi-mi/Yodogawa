...
AIネイティブIDE（Windsurf, Cursor, Antigravity, Claude Code）向けの**仕様駆動開発ワークフロー集**。Markdownベースのワークフロー定義を蓄積し、`npm install -g yodogawa` で他プロジェクトに導入できるCLIツールとして配布する。
...
npmパッケージ `yodogawa` のエントリポイント。対話形式でIDE種別（Windsurf/Antigravity/Cursor/Claude Code）を選択し、`.windsurf/` 配下のワークフロー・テンプレートをプロジェクトにコピーする。Cursor/Claude Code選択時は `workflows/` → `commands/` にリネームされる。

依存: `fs-extra`, `kleur`, `prompts`

### ワークフロー体系 (`.windsurf/workflows/`)

各Markdownファイルが1つのスラッシュコマンド定義。番号プレフィックスで開発フェーズを表す：
...
ワークフロー間は `Call /workflow-name` で連携する。

### テンプレート (`.windsurf/templates/`)
...
- YAML frontmatter不使用、Markdown本文のみ
...
`package.json` の `files` フィールドで `bin`, `.windsurf`, `README.md`, `CHANGELOG.md` のみを配布対象にしている。ワークフローやテンプレートの追加・変更時はバージョンを上げて `npm publish` する。
