...
npmパッケージ `yodogawa` のエントリポイント。対話形式でIDE種別（Antigravity/Cursor/Claude Code/Codex）を選択し、`skills/`, `templates/`, `scripts/` をそのまま `{IDE_DIR}/` 配下にコピーする。
...
インストール後は各IDEの `{IDE_DIR}/skills/{name}/SKILL.md` として認識される。
