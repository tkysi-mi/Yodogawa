...
- [x] **[P2] ✅完了** **ペルソナ定義**（#37） → 専用ファイルは新設せず `01-product-brief.md` の「ターゲットユーザー / 主要ペルソナ」を **ID 付きペルソナ表**（P-XXX / 種別 / 役割 / ゴール / 課題・ペイン / 利用文脈 / 任意でニーズの強さ）として構造化（MVP 軽量方針を維持）。`05-user-stories.md` に「ペルソナ」列を追加して役割を P-XXX で参照解決し、a-006（2.1）が US 役割 → ペルソナ表の trace を検証する。a-002b は役割が既存ペルソナに無い場合 Product Brief への追加を促す。専用 `08-personas.md` は引き続き任意（規模が大きい場合のみ）。
...
- [ ] **[P2] ⬜未対応** a-002 SKILL 手順7（全体レビュー）に lint 実行を組み込む。
