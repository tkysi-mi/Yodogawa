...
5. **ローカルテストの実行** — 型チェック / ユニットテスト / Lint
6. **エラーハンドリング** — 修正を試み、ユーザーガイダンスが必要なら報告
7. **ステップの完了マーク** — `c-implementation.md` のチェックボックスを `- [ ]` → `- [x]` に更新
8. **Git コミット（オプション）** — コミットメッセージは [examples/commit-and-pr.md](examples/commit-and-pr.md) を参照
...
- **必須ドキュメントが不足**: 「{ドキュメント名} が見つかりません。先に該当ワークフローを実行してください（タスク定義 → `/b-001` / リサーチ → `/b-002` / 実装タスクリスト → `/b-003`）。」
